...
class StockOptimizationService:
    ...
    async def calculate_safety_stock_optimization(
        ...
        try:
            ...
            sales_data = self._get_item_sales_data(item_id)
            ...
            for service_level in target_service_levels:
                ...
                annual_stockout_cost = (
                    Decimal(str(stockout_probability)) * 
                    Decimal(str(demand_stats['annual_demand'])) * 
                    profit_per_unit * 
    ...
    async def calculate_economic_order_quantity(
        ...
        try:
            ...
            annual_ordering_cost = Decimal(str(annual_demand / eoq)) * ordering_cost
            annual_holding_cost = Decimal(str(eoq / 2)) * holding_cost_per_unit
    ...
    async def _generate_item_recommendation(
        ...
        confidence_score = min(1.0, len(sales_data) / 30) if 'sales_data' in locals() else 0.5
