"""
Simple Tests for Universal Inventory Management Service
Basic functionality tests without complex relationships
"""
...
from schemas_universal import UniversalCategoryCreate, UniversalInventoryItemCreate, InventorySearchFilters
...
def db_session():
    ...
    # Clean up test data (delete movements first due to foreign key constraints)
    try:
        session.execute(text("DELETE FROM inventory_movements WHERE inventory_item_id IN (SELECT id FROM inventory_items_new WHERE sku LIKE 'TEST-%')"))
        session.execute(text("DELETE FROM inventory_items_new WHERE sku LIKE 'TEST-%'"))
        session.execute(text("DELETE FROM categories_new WHERE name LIKE 'Test%'"))
        session.commit()
    ...
    # Clean up after test (delete movements first due to foreign key constraints)
    try:
        session.execute(text("DELETE FROM inventory_movements WHERE inventory_item_id IN (SELECT id FROM inventory_items_new WHERE sku LIKE 'TEST-%')"))
        session.execute(text("DELETE FROM inventory_items_new WHERE sku LIKE 'TEST-%'"))
        session.execute(text("DELETE FROM categories_new WHERE name LIKE 'Test%'"))
...
def test_inventory_analytics(service, sample_user_id):
    ...
    assert analytics['total_value'] >= 3000  # 3 items * 10 qty * 100 cost
