...
from models_rbac import (
    ...
    role_permissions, permission_group_permissions
)
...
PERMISSION_GROUPS = [
    {
        ...
        "description": "Permissions for dashboard viewing and interaction",
        "color": "#3B82F6",
        "icon": "chart-bar",
    ...
    },
    {
        ...
        "color": "#7C2D12",
        "icon": "package",
        "permissions": ["inventory:view", "inventory:create", "inventory:update", "inventory:delete", "inventory:manage_categories", "inventory:stock_adjustment"]
    },
    {
        "name": "customer_management",
        "display_name": "Customer Management",
        "description": "Customer relationship management permissions",
        "color": "#2563EB",
        "icon": "users",
        "permissions": ["customers:view", "customers:create", "customers:update", "customers:delete", "customers:view_analytics"]
    },
    {
        "name": "financial_operations",
        "display_name": "Financial Operations",
        "description": "Financial and accounting permissions",
        "color": "#059669",
        "icon": "currency-dollar",
        "permissions": ["invoices:view", "invoices:create", "invoices:update", "invoices:delete", "invoices:approve", "invoices:send", "invoices:payment", "accounting:view", "accounting:create_entries", "accounting:update_entries", "accounting:delete_entries", "accounting:view_reports", "accounting:chart_of_accounts"]
    },
    {
        "name": "reporting_analytics",
        "display_name": "Reporting & Analytics",
        "description": "Business intelligence and reporting permissions",
        "color": "#7C3AED",
        "icon": "chart-line",
        "permissions": ["reports:view", "reports:create", "reports:export", "reports:schedule", "reports:share", "analytics:view", "analytics:advanced", "analytics:forecasting", "analytics:kpi"]
    },
    {
        "name": "system_administration",
        "display_name": "System Administration",
        "description": "System configuration and administration permissions",
        "color": "#DC2626",
        "icon": "cog",
        "permissions": ["settings:view", "settings:update", "settings:user_management", "settings:system_config", "settings:backup", "system:audit_logs", "system:health_check", "system:maintenance"]
    },
    {
        "name": "communication",
        "display_name": "Communication",
        "description": "SMS and communication permissions",
        "color": "#059669",
        "icon": "chat",
        "permissions": ["sms:view", "sms:send", "sms:manage_templates", "sms:campaigns"]
    }
]
...
async def create_permission_groups(db):
    ...
    for group_data in PERMISSION_GROUPS:
        ...
        result = await db.execute(
            select(RBACPermissionGroup).where(RBACPermissionGroup.name == group_data["name"])
        )
        existing = result.scalar_one_or_none()
    ...
    await db.commit()
    print(f"✅ Created {created_count} permission groups")
    ...
    for role_data in SYSTEM_ROLES:
        ...
        if not existing:
            ...
            if role_data["permissions"] == "all":
                ...
                all_perms_result = await db.execute(select(RBACPermission))
...
async def assign_default_roles(db):
    ...
    users_result = await db.execute(
        select(User).where(~User.rbac_roles.any())
    )
    users_without_roles = users_result.scalars().all()
...
async def seed_rbac_data():
    ...
    try:
        async with get_db_session() as db:
            ...
            # Create permission groups
            await create_permission_groups(db)
